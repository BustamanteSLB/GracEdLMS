...
const InstitutionAndroid: React.FC = () => {
        ...
        <View className="mt-[5px]">
          {links.map((item, index) => (
            <TouchableOpacity 
              ...
              >
              <Text className={`font-inter_regular justify-center text-left text-[14px] ${isDarkMode ? 'text-[#E0E0E0]' : 'text-black'}`}>{item.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <TouchableOpacity
          ...
          >
          <Text className={`${isDarkMode ? 'text-[#E0E0E0]' : 'text-black'}`}>Mission & Vision</Text>
        </TouchableOpacity>
        {isExpanded && (
...
export default InstitutionAndroid