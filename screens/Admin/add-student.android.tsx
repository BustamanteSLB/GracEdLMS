...
const AddStudentAndroid = () => {
  ...
  const handleAddStudent = async () => {
    ...
    // Basic validation for required fields for a new student
    ...
    try {
      const newStudentData = {
        ...
        status: 'active', // Default status for new student, adjust if 'pending' is needed
      };

      const response = await apiClient.post('/users', newStudentData); // Use POST for creation
      successMessages.push('Student account added successfully!');
      ...
      errorMessages.push(err.response?.data?.message || 'Failed to add student.');
    }
        ...
        <ScrollView contentContainerStyle={{ 
          ...
          <View className={`w-full rounded-xl p-4 ${isDarkMode ? 'bg-[#121212] shadow-none' : 'bg-white shadow-lg'}`}>
            ...
            <CustomButton
              ...
              title={submitting ? 'Adding...' : 'Add Student'}
...
export default AddStudentAndroid;