import { Text, View, useColorScheme, ActivityIndicator, TouchableOpacity, Modal, Alert } from 'react-native'
...
const UserManageAndroid = () => {
        ...
        <View style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }} className='flex-1 justify-center items-center'>
          ...
          >
            ...
            <View className='flex-row'>
              <TouchableOpacity
                className='bg-secondary-android rounded-xl h-[50px] justify-center items-center p-2 mr-2'
  ...
  const handleAddMultipleUsers = () => {
    ...
    Alert.alert(
      ...
      'Admin will submit JSON/CSV for multiple users.',
      [{ text: 'OK' }]
    );
            ...
            const deleteUser = async () => {
              ...
              try {
                ...
                if (response.data.success) {
                  Alert.alert('Success', 'User account archived successfully.', [{ text: 'OK' }]);
                  setUsers((prevUsers) =>
                    prevUsers.filter((user) => user._id !== userId)
                  );
                } else {
                  Alert.alert('Error', response.data.message || 'Failed to archive user.', [{ text: 'OK' }]);
                }
              } catch (err: any) {
  ...
  if (users.length === 0) {
    return (
      <SafeAreaView className={`flex-1 items-center justify-center ${isDarkMode ? 'bg-[#121212]' : 'bg-white'} `}>
        ...
        <CustomButton
          containerStyles='bg-[#60A5FA] h-[50px] p-2 mt-4'
          handlePress={handleOpenAddUserModal}
          title='Add User'
  ...
  return (
    <SafeAreaView className={`flex-1 ${isDarkMode ? 'bg-[#121212]' : 'bg-white'} `}>
      <View className='flex-row'>
        ...
        <TouchableOpacity
          className='bg-[#60A5FA] rounded-xl h-[50px] justify-center items-center ml-auto mr-2 p-2'
...
export default React.memo(UserManageAndroid)