...
const QuizzesWeb = () => {
  ...
  // New state variables for image preview
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      ...
      params.append('status', 'published'); // Only get published quizzes for students
  ...
  const closeQuizModal = () => {
    ...
    setQuizResults(null);
    setShowInstructions(false);
    setImagePreviewModal(false); // Close image preview modal
    setSelectedPreviewImage(null); // Clear selected image
    if (timerRef.current) {
      clearTimeout(timerRef.current);
    }
  };
        ...
        {question.images && question.images.length > 0 && (
          <View style={{ marginBottom: 16 }}>
            {question.images.map((image, imgIndex) => (
              <TouchableOpacity
                key={imgIndex}
                onPress={() => handleImagePress(`http://192.168.100.5:5000${image}`)}
                activeOpacity={0.8}
              >
                <Image
  ...
  const getItem = (data: Quiz[], index: number) => data[index];
  const getItemCount = (data: Quiz[]) => data.length;
                  ...
                  {timeLeft !== null && (
                    <Text
                      style={{
                        fontSize: 18,
                        fontFamily: 'Inter-24pt-Bold',
                        color: timeLeft <= 300 ? '#EF4444' : '#10B981', // Red when 5 minutes or less
                      }}
                    >
                      {formatTime(timeLeft)}
                    </Text>
