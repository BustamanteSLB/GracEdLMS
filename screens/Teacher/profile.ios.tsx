import { Text, ScrollView, View, useColorScheme, TextInput, TouchableOpacity, ActivityIndicator, RefreshControl } from 'react-native'
...
const ProfileIOS: React.FC = () => {
  ...
  if (authIsLoading || (!user && !isLoggingOut)) {
    return (
      <SafeAreaView className='flex-1 items-center justify-center bg-primary-ios'>
    ...
    <View className={`flex-1 items-center justify-start w-full h-full ${isDarkMode ? 'bg-[#121212]' : 'bg-white'}`}>
      ...
      >
        ...
        <View className='w-full p-4 rounded-lg'>
          ...
          <CustomButton
            containerStyles={`bg-secondary-ios h-[55px] mb-2`}
            title="Edit Profile"
