  ...
  return (
    <SafeAreaView className="flex-1 h-full bg-primary-web">
      <ScrollView contentContainerStyle={{ flexGrow:1, alignItems: 'center', justifyContent:'center', padding:16 }}>
        <View className={`w-full max-w-xl rounded-xl p-4 ${isDarkMode ? 'bg-[#121212] shadow-none' : 'bg-white shadow-lg'}`}>
          <Image
          ...
          {/* <View className={`overflow-hidden border rounded-xl mt-4 mb-2 ${isDarkMode ? 'border-[#1E1E1E] bg-[#1E1E1E]' : 'border-gray-300'}`}>
            ...
            >
              <Picker.Item label="Select Account Type" value="" style={{ color: isDarkMode ? '#E0E0E0' : 'black', fontFamily:'Inter-18pt-Regular'}} />
              {accountTypeOptions.map((option) => (
                <Picker.Item key={option.value} label={option.label} value={option.value} style={{ fontFamily: 'Inter-18pt-Regular' }}  />
              ))}
            </Picker>
          </View> */}
          <View className={`flex-row items-center border rounded-xl mt-2 mb-4 px-3 py-2 h-[55px]
