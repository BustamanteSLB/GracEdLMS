import { ActivityIndicator, Alert, Text, TouchableOpacity, View, VirtualizedList, useColorScheme } from 'react-native'
      ...
      const deleteStudent = async () => {
        ...
        try {
          ...
          if (response.data.success) {
            window.alert('Success: Student account archived successfully.');
            // Update the local state to remove the soft-deleted student
            setStudents((prevStudents) =>
              prevStudents.filter((student) => student._id !== studentId)
            );
          } else {
            window.alert('Error: ' + (response.data.message || 'Failed to archive student.'));
          }
        } catch (err: any) {
...
export default ManageStudentsWeb